...
export class RemoveFromCartResponseDto {
  ...
  /**
   * `장바구니 상품 삭제` 응답을 `장바구니 조회`의 응답과 동일하게 함
   * @param cart
   * @returns
   */
  static toResponseDto(cart: Cart): RemoveFromCartResponseDto {
    const responseDto = {
      id: cart.id,
    ...
    return plainToInstance(RemoveFromCartResponseDto, responseDto);
  }
}
