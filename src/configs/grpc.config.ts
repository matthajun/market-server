...
/**
 * media-file-upload 서버에 상품(기획) 행사 이미지 업로드 요청
 ...
 *
 */
export function getGrpcPlanningEventImageUploadClientOptions(): ClientOptions {
  const MEDIA_FILE_UPLOAD_SERVER_GRPC_ENDPOINT =
    process.env.MEDIA_FILE_UPLOAD_SERVER_GRPC_ENDPOINT;

  if (!MEDIA_FILE_UPLOAD_SERVER_GRPC_ENDPOINT) {
    throw new Error('MEDIA_FILE_UPLOAD_SERVER_GRPC_ENDPOINT env required');
  }

  return {
    transport: Transport.GRPC,
    options: {
      package: 'planning_event_image_upload_proto',
      url: MEDIA_FILE_UPLOAD_SERVER_GRPC_ENDPOINT,
      protoPath: join(
        __dirname,
        '../util/grpc/proto/planning_event_image_upload.proto',
      ),
